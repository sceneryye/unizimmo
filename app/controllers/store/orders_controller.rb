...
class Store::OrdersController < ApplicationController
 ...
 #立刻购买
  def express
    ...
    if product_id.nil?
      @product = Ecstore::Product.where(:goods_id=>goods_id).first
      product_id= @product.product_id
    end
    redirect_to "/orders/new?product_id=#{product_id}"
  ...
  def create
    ...
    if product_id.nil?
      ...
      @product = Ecstore::Product.find(product_id.to_i)
      @order.order_items << Ecstore::OrderItem.new do |order_item|
          order_item.product_id = @product.product_id
          order_item.bn = @product.bn
          order_item.name = @product.name
          ...
          order_item.goods_id = @product.goods_id
          order_item.type_id = @product.good.type_id
          order_item.nums = 1
          order_item.item_type = "product"
          order_item.amount = 10000#@product.price
