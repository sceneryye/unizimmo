...
class Store::GoodsController < ApplicationController
 ...
 def mproduct
  ...
  def show
    ...
    if @good.tag=='羊年团洋车'
      @current_price =  @good.price - @people*1000
    end
...
#记录用户商品查看日志
    if @user
      @review = Ecstore::GoodsReviewLog.where(:member_id=>@user.member_id,:goods_id=>@good.goods_id).first_or_initialize do |review|
        review.member_id = @user.member_id
        review.goods_id = @good.goods_id
      end
      if @review.new_record?
         @review.save
      else
        Ecstore::GoodsReviewLog.where(:member_id=>@user.member_id,:goods_id=>@good.goods_id).update_all(:times=>@review.times+1)
      end
    end

  
    @goods_pics =@good.big_pic.split(',')
