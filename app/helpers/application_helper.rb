...
module ApplicationHelper
	...
	def truncate2(content,options={})
      ...
      def page_title
        (content_for :title).present? ? (content_for :title) : '滋贸万国'
      end

      def meta_keywords
        (content_for :keywords).present? ? (content_for :keywords) : '滋贸万国'
      end

      def meta_description
        (content_for :description).present? ? (content_for :description) : '滋贸万国'
      end
